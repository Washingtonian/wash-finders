...
use App\Filament\Resources\RentalPropertyProviderResource\Pages\CreateRentalPropertyProvider;
use App\Filament\Resources\RentalPropertyProviderResource\Pages\EditRentalPropertyProvider;
use App\Filament\Resources\RentalPropertyProviderResource\Pages\ListRentalPropertyProviders;
...
class RentalPropertyProviderResource extends Resource
{
    protected static ?string $model = Provider::class;

    protected static BackedEnum|string|null $navigationIcon = 'heroicon-o-building-office-2';

    protected static ?string $navigationLabel = 'Rental Properties';

    protected static ?string $modelLabel = 'Rental Property';

    protected static ?string $pluralModelLabel = 'Rental Properties';

    protected static UnitEnum|string|null $navigationGroup = 'Providers';

    public static function form(Schema $schema): \Filament\Schemas\Schema
    {
        return $schema
            ->schema([
                ...
                FileUpload::make('photo')
                    ->label('Photo')
                    ->image()
                    ->directory('enhanced_photos/rental_properties')
                    ->visibility('public'),
                KeyValue::make('meta')
                    ->keyLabel('Key')
                    ->valueLabel('Value')
    ...
    {
        return $table
            ->modifyQueryUsing(fn (Builder $query) => $query->where('type', 'rental_properties'))
            ->columns([
    ...
    {
        return [
            'index' => ListRentalPropertyProviders::route('/'),
            'create' => CreateRentalPropertyProvider::route('/create'),
            'edit' => EditRentalPropertyProvider::route('/{record}/edit'),
        ];
    }
    ...
    {
        $parameters['type'] = 'rental_properties';
